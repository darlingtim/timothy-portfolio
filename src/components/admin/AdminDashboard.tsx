    ...
    <div className="min-h-screen flex bg-slate-100 dark:bg-[#070d1e] text-slate-900 dark:text-slate-100 font-sans">
      ...
      >
        ...
        <div className="p-5 border-b border-slate-800 flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            {isDemoMode ? (
              <span className="inline-flex items-center gap-1 text-[11px] text-amber-400 font-mono tracking-wider font-semibold">
                <span className="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse"></span>
                Demo Sandbox
              </span>
            ) : (
              <span className="inline-flex items-center gap-1 text-[11px] text-emerald-400 font-mono tracking-wider font-semibold">
        ...
        <div className="p-4 border-t border-slate-800 space-y-2 text-sm">
          {isDemoMode && onOpenAdminLogin && (
            <button
              onClick={onOpenAdminLogin}
              className="w-full flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl bg-gradient-to-r from-sky-600 to-blue-600 hover:from-sky-500 hover:to-blue-500 text-white font-semibold shadow-md transition-all text-xs"
            >
          ...
          >
            ...
            <span className="font-medium">{isDemoMode ? 'Exit Admin View' : 'Logout'}</span>
          </button>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-16 bg-white dark:bg-[#0c142c] border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-4 sm:px-6 lg:px-8 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              {isDemoMode ? (
                ...
                <span className="px-2 py-0.5 rounded-md text-[10px] font-mono font-bold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 flex items-center gap-1">
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            {isDemoMode && onOpenAdminLogin && (
              <button
                onClick={onOpenAdminLogin}
                className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-gradient-to-r from-sky-600 to-blue-600 hover:from-sky-500 hover:to-blue-500 text-white shadow-sm transition-all"
              >
                <Lock className="w-3.5 h-3.5" />
                <span>Admin Login (2FA)</span>
              </button>
            )}
            ...
            >
              ...
              <div className="hidden md:block text-left">
                ...
                <span className="text-[10px] text-sky-500 font-mono">
                  {isDemoMode ? 'Visitor / Demo' : 'Administrator'}
                </span>
              </div>
            </div>
          </div>
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 space-y-8">
          ...
          {isDemoMode && (
            <div className="p-4 sm:p-5 rounded-2xl bg-gradient-to-r from-amber-500/10 via-sky-500/10 to-indigo-500/10 border border-amber-500/30 dark:border-amber-500/20 backdrop-blur-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-sm animate-in fade-in">
              <div className="flex items-start gap-3.5">
                <div className="w-10 h-10 rounded-xl bg-amber-500/20 border border-amber-500/30 text-amber-500 flex items-center justify-center shrink-0 mt-0.5">
                  <Shield className="w-5 h-5" />
                </div>
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    <h4 className="text-sm font-bold text-slate-900 dark:text-white">
                      Visitor Demo Sandbox Active
                    </h4>
                    <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-amber-500/20 text-amber-600 dark:text-amber-400 border border-amber-500/30">
                      Read-Only &amp; Safe Playground
                    </span>
                  </div>
                  <p className="text-xs text-slate-600 dark:text-slate-300 mt-1 max-w-2xl leading-relaxed">
                    You can test all forms, tabs, media library, and skill editors in this sandbox preview. Real incoming visitor messages and sensitive contact information are securely shielded, and live server database changes require two-factor authentication.
                  </p>
                </div>
              </div>
              ...
              {onOpenAdminLogin && (
                <button
                  onClick={onOpenAdminLogin}
                  className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-sky-600 to-blue-600 hover:from-sky-500 hover:to-blue-500 text-white text-xs font-bold shadow-md flex items-center gap-2 transition-all shrink-0 w-full sm:w-auto justify-center"
                >
                  <Lock className="w-3.5 h-3.5" />
                  <span>Authenticate as Admin (2FA)</span>
                </button>
              )}
            </div>
          )}
          ...
          {activeTab === 'messages' && (
            <div className="space-y-6 animate-in fade-in max-w-5xl">
              {isDemoMode && (
                <div className="p-4 rounded-xl bg-sky-950/40 border border-sky-800/60 text-sky-200 text-xs flex items-start gap-3">
                  <ShieldCheck className="w-5 h-5 text-sky-400 shrink-0 mt-0.5" />
                  <div className="space-y-1">
                    <p className="font-semibold text-white">
                      Visitor Privacy &amp; Inquiry Protection
                    </p>
                    <p className="text-slate-300 leading-relaxed">
                      Real inbound contact messages, inquiries, and visitor emails are shielded from public view in Demo Sandbox mode. Authenticate via 2FA as the site administrator to view and send live direct email replies.
                    </p>
                  </div>
                </div>
              )}
